<template>
  <el-dialog title="角色授权限制" :visible.sync="dialogVisible" size="tiny"  class="role-atuh-dialog-wrapper">
  ...
  export default {
    ...
    methods: {
      ...
      show ({roleId}) {
        ...
        this.checkedKeys = [];
        let type = '0';
        this.$api.bsp.role.getResourceTreeData4User(roleId, type).then((ret) => {
      ...
      sure () {
        ...
        this.loading = true;
        let type = '0';
        this.$api.bsp.resource.authResourceToRole(this.roleId, resIds.join(","), type).then(ret => {
