        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="应用">
              <app-option ref="appOption" v-model="filterNotice.appId"></app-option>
            </el-form-item>
          </el-col>
        ...
        <pl-content-box-block>
          <pl-page-table :data-fun="getUsersData" ref="pageTable" :params="filterNotice">
            <el-table-column prop="title" label="标题" header-align="center"
                             sortable="title" align="center"></el-table-column>
            ...
            <el-table-column  prop="front" label="是否置顶" width="160" align="center">
              <template scope="scope">
                <span>{{ scope.row.front | constant('PubConstants.YesOrNo')}}</span>
              </template>
            </el-table-column>
            <el-table-column  prop="noticeState" label="公布状态" width="160" align="center">
              <template scope="scope">
                <span>{{ scope.row.noticeState | constant('GovernorConstants.NoticeState')}}</span>
              </template>
            </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addNotice () {
        let appId = this.filterNotice.appId;
        if (!appId) {
          this.$msgbox({
            title: '',
            message: '请选择应用'
        ...
        this.$router.push({name: 'bsp.biz.notice-add-edit', params: {isEdit: '0', appId: appId}});
