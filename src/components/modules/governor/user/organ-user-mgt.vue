<template>
  <pl-content-box>
    ...
    <pl-content-box-toolbar>
         ...
         <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="登录名">
              <el-input v-model="filterUser.loginName" placeholder="请输入关键字"></el-input>
            </el-form-item>
          </el-col>
      ...
      <el-row>
        ...
        <pl-content-box-block>
          <pl-page-table :data-fun="getUsersData" ref="pageTable" style="width:100%">
            ...
            <el-table-column  prop="userRoleNames" label="所属角色" width="150"></el-table-column>
            <el-table-column  prop="userState" label="状态" width="100">
              <template scope="scope">
                <span >{{ scope.row.userState | constant('PubConstants.State')}}</span>
              </template>
            </el-table-column>
            <el-table-column  prop="phoneNo" label="联系电话" width="150"></el-table-column>
            <el-table-column  prop="certType" label="证件类型" width="150">
              <template scope="scope">
                <span >{{ scope.row.certType | dictionary('id_type')}}</span>
              </template>
            </el-table-column>
            <el-table-column  prop="certNo" label="证件号码" width="150"></el-table-column>
            <el-table-column  prop="organName" label="所属部门" width="100"></el-table-column>
            <el-table-column  prop="email" label="电子邮箱" width="150"></el-table-column>
            <el-table-column  prop="note" label="描述" width="150"></el-table-column>
            <el-table-column   label="操作" width="260" fixed="right" header-align="center" align="center">
              <template scope="scope">
                <el-button-group>
                    ...
                    <el-button size="mini" :loading="stateLoading" v-if="scope.row.userState === 'E'" v-tooltip:hover.top="'禁用'"
                    icon=" anticon icon-minuscircleo" @click="updateState(scope.$index,scope.row,'E','禁用')">
                    </el-button>
                    <el-button size="mini" icon=" anticon icon-solution" v-tooltip:hover.top="'授权'"
                    ...
                    <el-button size="mini" icon=" anticon icon-retweet" v-tooltip:hover.top="'重置密码'"
    ...
    <user-auth ref="userAuthDialog" ></user-auth>
  </pl-content-box>
</template>
  ...
  export default {
    data () {
      return {
        ...
        getUsersData: this.$api.bsp.user.selectOrganUsersByPage,
        organs: [],
    ...
    created () {
      this.$api.bsp.organ.getOrganTreeData().then((ret) => {
        ret = ret.filter(x => x.organState === 'E');
        if (typeof ret === 'string') {
          this.organs = JSON.parse(ret);
        } else {
          this.organs = ret;
        }
      });
