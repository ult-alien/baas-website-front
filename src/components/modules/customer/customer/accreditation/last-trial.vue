  ...
  <pl-content-box>
        ...
        <el-row>
          ...
          <el-col :span="6" label-width="80px">
            <el-form-item label="状态">
              <pl-constant-select type="CustomerConstants.CusState"  invalidKeys="U,D,0,1,2,5,E" v-model="params.cusState" :showEmpty="true" ></pl-constant-select>
            </el-form-item>
          </el-col>
    ...
    <pl-content-box-block>
      <pl-page-table :data-fun="getCusData" :params="params" ref="pageTable" style="width:100%" default-sort-prop="cus_name" default-sort-order="descending">
        ...
        <!--<el-table-column  prop="certNum" :show-overflow-tooltip="true" label="申请证书数量" width="120"></el-table-column>-->
        ...
        <el-table-column   label="操作" width="80"  align="center" header-align="center">
          <template slot-scope="scope">
            <el-button-group>
                <el-button size="mini" icon=" el-icon-edit" v-if="scope.row.cusState === CusState['WAIT_LAST_TRIAL']" v-tooltip:hover.top="'认证审核'"
                            @click="handleEdit(scope.row)"></el-button><!-- 修改 -->
                <el-button size="mini" icon="anticon icon-ellipsis" v-if="scope.row.cusState !== CusState['WAIT_LAST_TRIAL']" v-tooltip:hover.top="'详情'"
...
<script>
  ...
  export default {
    data () {
      return {
        getCusData: this.$api.cust.accreditation.getLastTrialCus,
        CusState: constantUtil.getConstantByKey('CustomerConstants.CusState'),
        params: {
          cusType: '',
          cusState: constantUtil.getConstantByKey('CustomerConstants.CusState')['WAIT_LAST_TRIAL'],
          cusName: ''
        }
      };
    },
