    ...
    <pl-content-box-block :fixed-width="false">
      ...
      <div class="accreditation-content">
        ...
        <el-form-item label="证件类型" prop="[customerC][idType]" :rules="{
          ...
          <pl-dictionary-select type="ent_id_type" v-model="accreditationForm.customerC.idType" :showEmpty="false"  @change="handleChangeIdType"></pl-dictionary-select>
        </el-form-item>
        ...
        <el-form-item label="营业执照" prop="[customerC][lisNo]" v-if="accreditationForm.customerC.idType === 'ent_id_type_2'" :rules="{
            trigger: 'change', required: true, message: '营业执照不能为空'
          }">
          <el-input v-model="accreditationForm.customerC.lisNo" tips="一般由15或13位数字组成" placeholder="请输入营业执照"></el-input>
        </el-form-item>
        ...
        <el-form-item label="证件名称" prop="[customerC][idName]" v-if="accreditationForm.customerC.idType === 'ent_id_type_3'" :rules="{
        ...
        <el-form-item label="证件号码" prop="[customerC][idNo]" v-if="accreditationForm.customerC.idType === 'ent_id_type_3'" :rules="{
...
<script>
  ...
  export default {
    ...
    methods: {
      handleChangeIdType () {
        this.accreditationForm.customerC.socCrCode = '';
        this.accreditationForm.customerC.lisNo = '';
        this.accreditationForm.customerC.orgCode = '';
        this.accreditationForm.customerC.idName = '';
        this.accreditationForm.customerC.idNo = '';
        this.$refs.accreditationForm.clearValidate();
      },
      changeIdType () {
      ...
      validateInviteCode (rule, value, callback) {
        if (!value || value === '') {
          ...
          // 校验验证码
          this.$api.cust.accreditation.checkInviteCode({'inviteCode': value}).then(ret => {
            if (!ret) {
