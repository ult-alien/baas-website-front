<template>
  <div  style="min-height: 700px;" v-loading="pageLoading">
    <pl-content-box-toolbar v-if="!pageLoading">
      <el-steps :active="step":align-center="true" :center="true"  finish-status="success" class="myprogress">
...
<script>
  ...
  import plFooter from '../../../governor/uiComponents/footer/footer-w.vue';
  ...
  export default {
    data () {
      return {
        ...
        pageLoading: true,
        needInviteCode: true,
        contractOpen: '', // 契约互联服务用户开户声明书
        contractAuth: '', // 契约互联服务账户授权确认书
        cusAuth: '' // 企业授权书
      };
    },
    methods: {
    ...
    beforeMount () {
      ...
      this.$api.cust.accreditation.getCustomerInfo().then(ret => {
        ...
        this.accreditationForm['operators'] = [];
        // 待提交认证状态才显示服务协议对话框
