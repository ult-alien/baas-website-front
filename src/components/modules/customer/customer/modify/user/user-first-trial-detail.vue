        ...
        <pl-content-box-toolbar class="text-center" :border="false">
          ...
          <div class="apr-block" v-if="showOpt">
            <el-form label-width="80px" ref="form" :model="form" style="width: 350px;margin: 0px auto;" class="static-form">
              <el-row>
                <el-col :span="24" v-if="showLastOpt">
                ...
                <el-col :span="24">
                  <el-form-item label="初审结果" prop="result" :rules="[
...
<script>
  ...
  let bizCode = constantUtil.getConstantByKey('PubConstants.UserBizCode');
  ...
  export default {
    ...
    computed: {
      showLastOpt () {
        if (this.lastOpt && bizCode['LAST_TRIAL'] === this.lastOpt.bizCode) {
          return true;
        }
        return false;
      },
    ...
    methods: {
      goBack () {
        this.refershMsgTip('user-first-apr');
        this.$router.push({name: 'bsp.biz.cus-modify-trial', params: {tab: 'user'}});
      },
      pass () {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            this.passLoading = true;
            this.$api.bsp.user.firstTrialPass(this.form).then(ret => {
      ...
      reject () {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            this.$api.bsp.user.firstTrialReject(this.form).then(ret => {
